...
// option: -w, --wad
//
static int handle_wad(char const *opt, int optf, int argc,
                      char const *const *argv)
{
   if (!argc) option::exception::error(opt, optf, "requires argument");

   Lump::create_wad(argv[0]);

   return 1;
}
static option::option_call
option_wad('w', "wad", "input", "Adds a WAD's lumps.", NULL, handle_wad);

//
// Lump::create_wad
//
void Lump::create_wad(char const *filename)
{
   unsigned char *data;
   ...
   // Try to load the data using mmap or read.
   {
      ...
      data = static_cast<unsigned char *>(mmap(NULL, s.st_size, PROT_READ,
                                               MAP_PRIVATE, fd, 0));
   ...
   {
      FILE *wad = fopen(filename, "rb");
      if (!wad) Exception::error("unable to fopen '%s'", filename);
   ...
   size_t hcount = data[ 4] <<  0L |
                   data[ 5] <<  8L |
                   data[ 6] << 16L |
                   data[ 7] << 24L;
   size_t offset = data[ 8] <<  0L |
                   data[ 9] <<  8L |
                   data[10] << 16L |
                   data[11] << 24L;
   ...
   {
      size_t start = header[0] <<  0L |
                     header[1] <<  8L |
                     header[2] << 16L |
                     header[3] << 24L;
      size_t len   = header[4] <<  0L |
                     header[5] <<  8L |
                     header[6] << 16L |
                     header[7] << 24L;
      LumpName name = Lump::name_from_string(reinterpret_cast<char *>(header+8));

      Lump::create_data_keep(name, reinterpret_cast<char *>(data+start), len);
   }
